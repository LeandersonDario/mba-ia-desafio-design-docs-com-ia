...
flowchart LR
    ...
    subgraph W["Processo Worker (src/worker.ts, polling 2s)"]
        ...
        P -->|"retentativas esgotadas"| DLQ[("webhook_dead_letter")]
    end
```
...
Semântica adotada: as "5 tentativas" da reunião são as **5 retentativas agendadas com os 5 delays** (1m/5m/30m/2h/12h) após a falha do envio inicial — é o que realiza o "total de quase 15 horas entre primeira falha e última tentativa" ([09:17] Diego). O evento só vai para a DLQ quando a retentativa agendada com o delay de 12h também falha.

```ts
const BACKOFF_MS = [60_000, 300_000, 1_800_000, 7_200_000, 43_200_000] as const;
// índice = attempts - 1: delay aplicado após a falha de número `attempts`,
// agendando a retentativa correspondente (1ª falha → +1min, …, 5ª falha → +12h)

async function handleFailure(tx, event, error): Promise<void> {
  const attempts = event.attempts + 1; // falhas acumuladas (envio inicial + retentativas)
  if (attempts > BACKOFF_MS.length) {
    // a 5ª retentativa (agendada com o delay de 12h) também falhou — falha permanente
...
| Envio | Delay após a falha anterior | Tempo acumulado desde a 1ª falha |
|---|---|---|
| Tentativa inicial | — (imediata, no ciclo de polling) | 0 |
| Retentativa 1 | 1 min | ~1 min |
| Retentativa 2 | 5 min | ~6 min |
| Retentativa 3 | 30 min | ~36 min |
| Retentativa 4 | 2 h | ~2h36 |
| Retentativa 5 | 12 h | ~14h36 ("quase 15 horas" — [09:17] Diego) |
| DLQ | após a falha da retentativa 5 | — |

### 5.4 DLQ e replay admin

Esgotadas as retentativas (a 5ª retentativa, agendada com o delay de 12h, também falhou — §5.3), o evento vai para tabela separada `webhook_dead_letter` com payload, motivo e timestamp ([09:18] Diego), em transação: cria a linha na DLQ e marca a linha da outbox como falha permanente (removendo-a do ciclo de leitura do worker — "mais limpa a leitura da outbox principal", [09:18] Diego).
...
**Logs estruturados (Pino existente — `src/shared/logger/index.ts`)** — decisão de reuso: "o logger, que é Pino, já tá no projeto inteiro. Não vamos botar nada novo" ([09:29] Bruno). O worker cria seu logger a partir do mesmo módulo — via `logger.child({ service: 'order-management-worker' })` ou parametrizando `createLogger()` (que hoje não recebe argumentos e fixa `service: order-management-api`, portanto essa segunda opção implica pequena alteração no arquivo compartilhado — derivado). **A secret do endpoint NUNCA pode ser logada**: adicionar `*.secret` e `*.previousSecret` aos `redactPaths` existentes (`*.password`, `*.token`, ...) — prática derivada diretamente do mecanismo de redaction já presente no logger, motivada pelo incidente real de secret vazada em log ([09:22] Diego).

**Tracing — correlação por `event_id`** (não há stack de tracing distribuído no projeto; o rastreamento distribuído formal fica como evolução — nesta fase o tracing é feito por correlação de identificadores, derivado do padrão `requestId` do `request-logger.middleware.ts` existente): o `event_id` atravessa toda a cadeia — nasce na inserção na outbox (API), aparece nos logs do worker, no header `X-Event-Id` e em `webhook_deliveries.outboxEventId` — permitindo rastrear uma entrega de ponta a ponta. Eventos de log sugeridos: `webhook_event_enqueued`, `webhook_delivery_attempt`, `webhook_delivered`, `webhook_delivery_failed`, `webhook_moved_to_dlq`, `webhook_dlq_replay`.
...
- **Novo processo**: `src/worker.ts` + script `"worker": "tsx watch --env-file=.env src/worker.ts"` (dev) e `node --env-file=.env dist/worker.js` (produção, espelhando o `start` existente) no `package.json`, análogo aos scripts `dev`/`start` existentes ([09:11] Larissa: "criar um src/worker.ts e um script npm run worker").
...
- [ ] **DLQ após esgotar as retentativas**: a falha da 5ª retentativa (a agendada com delay de 12h) move o evento para `webhook_dead_letter` com payload, motivo e timestamp, e ele sai do ciclo do worker ([09:18] Diego).
...
| `tests/webhook-worker.test.ts` | Loop/processor com endpoint fake | Servidor HTTP local efêmero (`node:http`) como endpoint do cliente; invoca `processEvent`/um `tick()` diretamente (sem esperar polling real); cenários: 200 ⇒ `DELIVERED` + linha em `webhook_deliveries`; 500 ⇒ `FAILED`, `attempts = 1`, `nextRetryAt ≈ +1min`; retentativas esgotadas ⇒ linha na `webhook_dead_letter` e replay via endpoint admin reenfileira |
